...
2. **Planned Entry & Position Sizing**: Enter a planned entry price and current ATR(14) for a shortlisted ticker. Position size targets **1% of NEPSE strategy capital**, then caps the result to what available cash can actually fund. Strategy capital includes only money assigned to this tracker; record money moved elsewhere as a withdrawal. The calculator also shows cash concentration, a minimum-lot-size warning below 10 shares, and an optional liquidity check.
3. **Save Trade Plan**: A plan can be saved on any day; the app never sends it to NEPSE or a broker. Saving is blocked if all 5 portfolio slots are committed, the ticker already has a plan or position, there isn't enough cash for the sized position, or the Distribution Day Counter is hard-gated (see Step 0). Submit the day order yourself through your broker's TMS during an open NEPSE session. After each trading day, log the close, fresh ATR(14), and any fills; while the market remains confirmed, the tracker calculates the next session's price, stop, and affordable size. A log is idempotent per trading date, so repeated clicks cannot duplicate fills or consume attempts. The tracker closes the plan if unfilled after 5 daily attempts, when a day's close breaks that day's current stop, or when the market gate closes. Partial fills accumulate toward a running fill VWAP across multiple days and retain their first-fill date.
...
- **Distribution Day Counter**: Upload a NepseAlpha index CSV export or paste bars manually (`Date Close Volume`, one per line); tolerant of header rows and extra whitespace. Tracks the trailing distribution-day count, detects Follow-Through Days, and hard-blocks new trade plans once the count is severe.
- **Strategy Capital and Available Cash**: Strategy capital is the stable risk-sizing base; available cash is the separate affordability cap. Realized P&L updates strategy capital, while deposits and withdrawals can be recorded manually.
...
- **Pending Trade Plans**: Log each trading session's close, ATR, and any shares actually filled in your TMS. The tool handles date-idempotent logging, calculates the next session's plan while the market remains confirmed, caps size to available cash, closes plans at the stop/macro gates or 5-day limit, and converts completed/partial fills into an active trade.
...
- **Historical Log & P&L**: Closed trades are archived with entry/exit price, shares, risk taken, P&L, and return %; sale proceeds return to available cash as each tranche is logged.
...
- **Automatic NEPSE Holiday Calendar**: A scheduled GitHub Action syncs the official holiday listing into `holidays.json` for session-dependent logs and streaks; trade plans can still be saved if the calendar is unavailable.
...
- **Max Risk per Position**:
  $$\text{Max Risk} = \text{NEPSE Strategy Capital} \times 1\%$$
...
- Portfolio is fixed at 5 slots and targets 1% modeled risk per position (up to roughly 5% when all slots were sized from the same strategy-capital base); fees, slippage, and gaps can make realized risk differ.
